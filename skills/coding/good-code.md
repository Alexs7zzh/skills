...
- **Logic lives in its canonical layer.** Apply the deletion test to every helper, wrapper, mode, check, flag, or dependency: delete it mentally and name the requirement that fails, and where that requirement lives, in a recorded ruling or spec, the project's values, behavior users depend on, or the domain's common sense. Complexity reappearing at N call sites means it earns its keep. Nothing fails, or the requirement lives nowhere, means mass rather than structure, however correct: its removal is a Restructure, or a question for the user when keeping it turns on a value nobody has recorded.
...
**Structural quality.** Changed code only, never pre-existing code. Three shapes fail the deletion test on sight: a gate whose only input is the actor's own declaration, a second copy of knowledge the code already holds, such as a schema restating the types or a caller re-deriving state, and a defense against a threat the code admits it cannot stop. New ad-hoc conditionals inserted into unrelated flows are design problems rather than nits, so name the abstraction, state machine or policy the logic belongs in. A diff that grows a file past its current order of magnitude asks for decomposition first. Look for the reframe that makes whole branches or layers disappear while preserving behavior. A quality finding names a concrete future cost, not a taste.
...
- every check, flag, mode, config key, dependency, and single-caller abstraction the change adds: the requirement that fails without it and where that requirement lives. None named is a Restructure whose proposal is its deletion
