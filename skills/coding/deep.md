...
- **Gather.** Follow the route's Gather the input section. Freeze the target revision, working-tree diff, and available artifacts. Do not decide causes or design fixes while assembling input. Choose a recoverable shelve form from the project convention or supported VCS; ask only when a material constraint prevents choosing one. Initialize a joint database with both reviewers, yourself, and the known input scopes so the cold passes inherit them; a scope list only in the dispatch does not populate the database.
- **Dispatch.** Start the reviewers per Herdr runtime. Give them the frozen input, checkout, record, and actual user constraints. Bind their runtime identities and your own to the coordinator after dispatch, then enable it. Tell the user that consequential questions appear as they arise and status is available.
...
- **Report status.** Use `ledger status` and the coordinator's observed runtime states. Separate recorded activity from inference; ready work does not prove that an agent is executing it, and the shared queue excludes unfinished private discovery. Surface blocked work and what would unblock it.
...
- Between work items, pull from `ledger status`: open coverage, candidate development, claims to check, candidate reviews, and conditions on your work. Do not wait for a peer to assign work already available in the record. Keep issue-to-candidate continuity; resume another investigator's retained work through ledger.md, Cooperative handoff.
...
**Preflight.** Check `HERDR_ENV=1` and that `herdr agent list` succeeds from the coordinator's execution context. If a sandbox denies runtime access, obtain normal approval for that context; do not bypass the sandbox. Reviewers need access to the record and their authorized work, not runtime permission for every ledger mutation. If runtime access is unavailable, use the local role and state the missing two-family capability.
...
**Peer communication.** Choose whether an interruption is needed before choosing a messaging tool. Findings, evidence, disagreements, candidates, and routine disclosures go directly into the shared record and retained artifacts; do not try a peer message first or send a duplicate summary. Busy peers pull that work between tasks. Interrupt for an urgent stop, possible interference with active work, or changed instructions that cannot wait for the next pull; retain the resulting decision in the record. A clarification needs a message only when the record cannot answer it and work cannot safely proceed without it. In a Herdr run, use the Herdr agent surface for these peer interruptions: harness-local subagent messaging addresses only that harness's registered agents. Ledger mutations need no runtime access; this is not a ban on urgent runtime messages. If an urgent contact fails, preserve the stop and surface the failure to the user rather than treating a saved note as confirmed notice. On a work notification, read the current state in its named run before acting: it may be outdated and grants neither ownership nor a fresh-reader assignment. Use ledger.md, Fresh reviews, for the arranger and existing child state.

**Coordinator.** Run one `ledger coordinate watch` process in an addressable background pane with runtime permission. Its state and observation history stay beside the work record. The pinned helper's help supplies binding, pause, resume, status and reconciliation commands. Coordination starts paused. Only the master enables it after the named roles are dispatched and bound; replacing a session requires an explicit binding. A watcher restart never clears a pause or an uncertain attempt. Keep its pane and process visible so an unavailable coordinator can be distinguished from idle reviewers.

**Wake-ups.** Ledger mutations save work and notification intent without contacting Herdr. The coordinator checks current eligible work and runtime state, including private work for an unimported seat. It combines pending work into a wake-up for a bound idle or done agent; it does not send shared conclusions into a cold pass. Busy agents keep working and pull from the record. Blocked, unknown, missing or replaced sessions are not idle. Runtime observations are evidence of activity, not proof of progress or completion.

**Limits and stops.** A runtime prompt cannot guarantee exactly-once receipt or prevent the target becoming busy between inspection and send. Persist each attempt before sending. An uncertain result stops automatic delivery until the master checks the recipient/runtime and records a disposition. An accepted wake does not complete work. A reviewer returning idle with unchanged duties after observed activity, or an unavailable reviewer with outstanding duties, creates a master attention item instead of an endless reviewer prompt loop. The coordinator wakes an idle master once for the outstanding condition; a busy master reads coordinator status, and recovery removes the condition. A durable pause suppresses all automatic wakes. Before a workload stop, pause the coordinator, then stop the affected agents directly and preserve their work and children. Pausing prevents subsequent sends, not a prompt already in flight. Resume only on the authorized repair path; do not replay the saved mutation.

**Idle time.** Work until nothing is ready, hand off, then go idle. The coordinator's machine polling supplies wake-ups without model polling or peer acknowledgment rounds. A handoff does not mean the task is clean or discard unresolved work. If the coordinator or provider stops, retained state permits recovery but cannot itself restart a process; the master owns that runtime failure.
