...
**Peer communication.** The shared record and its retained artifacts carry findings, evidence, disagreements, candidates, and next actions; messages draw attention to that work. Save routine results there without sending a second summary or asking a peer to acknowledge them. Use direct messages for urgent stops, changed instructions affecting active work, or clarifications the record does not answer; retain resulting decisions in the record. On a work notification, read the current state in its named run before acting: a message may be outdated and does not itself grant ownership or assign a fresh reader. Use ledger.md, Fresh reviews, for the arranger and existing child state.
...
**Limits and stops.** A runtime prompt cannot guarantee exactly-once receipt or prevent the target becoming busy between inspection and send. Persist each attempt before sending. An uncertain result stops automatic delivery until the master checks the recipient/runtime and records a disposition. An accepted wake does not complete work; repeated idle-with-unchanged-work is surfaced rather than becoming an endless prompt loop. Before a workload stop, pause the coordinator, then stop the affected agents directly and preserve their work and children. Pausing prevents subsequent sends, not a prompt already in flight. Resume only on the authorized repair path; do not replay the saved mutation.
