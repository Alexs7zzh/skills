...
A rejected argument or precondition is not by itself a broken mechanism. Read the pinned helper's error and help, correct an unsupported invocation, and continue when no mutation was saved. For example, the seat comes from `LEDGER_ME`, not a `status B` argument. If the outcome is uncertain, inspect the record before retrying. Stop and urgently notify the master for lost or uncertain delivery, conflicting ownership, duplicate dispatch, or a valid operation that cannot progress because coordination is broken; a working note alone does not deliver that stop.
...
Only the named arranger starts that reader. The dispatch carries the target revision, review basis and supporting inputs, including the saved candidate and baseline when reviewing a candidate. Retain those inputs, the child context identifier, and whether it is running, completed, or stopped in the arranger's working note. On resumption, use that record before starting another reader. The helper lists assessments still needed, not whether a child is already running. If inputs change while the child runs, carry the changed basis and inputs to that child for reassessment per findings.md, Continuity; a changed basis is not a request for a duplicate reader. If a child fails or stops, preserve its partial work and state before replacing it; do not run two readers for the same pending assessment. Continue unrelated ready work while a child runs, then hand off and resume on its result.
...
One writer holds the shared checkout at a time. Acquire it successfully with `checkout take` before the first shared edit; a status observation, however recent, grants no ownership. This includes docs and changes exempt from builds. State what you are changing; a holder finishes and records its current batch before releasing. Others take work that does not depend on reading a moving tree.

If you write under another owner's hold, stop further shared writes, including a self-directed rollback. Preserve your diff outside the checkout and urgently notify the holder and master per deep.md, Peer communication, so they can check interference before recovery. Matching a baseline afterwards does not prove another writer's work was preserved.
