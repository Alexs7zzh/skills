...
test("checkout wait uses current availability without clearing real external or user waits", () => {
  ...
  assert.match(cli("A", ["dispatch", "reserve", "child-write-batch", "task=write-batch", "taskRev=1", "inspectAfter=2099-01-01T00:00:00Z"], 1), /waiting on checkout/)
  cli("A", ["dispatch", "reserve", "child-independent-read", "task=independent-read", "taskRev=1", "inspectAfter=2099-01-01T00:00:00Z"])
  ...
  cli("A", ["dispatch", "reserve", "child-write-batch", "task=write-batch", "taskRev=1", "inspectAfter=2099-01-01T00:00:00Z"])
  assert.match(cli("A", ["dispatch", "reserve", "child-external-result", "task=external-result", "taskRev=1", "inspectAfter=2099-01-01T00:00:00Z"], 1), /waiting on external/)
  assert.match(cli("A", ["dispatch", "reserve", "child-user-choice", "task=user-choice", "taskRev=1", "inspectAfter=2099-01-01T00:00:00Z"], 1), /waiting on user/)
})
...
test("resumed owner publishes a version comparison and peer agreement reaches master", () => {
  const { cli, snapshot } = fixture()
  cli("A", add("compare"))
  cli("A", ["task", "release", "compare", "rev=1", "note=baseline retained", "next=compare candidate"])
  cli("B", ["task", "claim", "compare", "rev=2"])
  ...
  cli("B", ["task", "publish", "compare", "rev=3", "disposition=done", "result=comparison@1"])
  assert.deepEqual(snapshot().state.tasks[0]!.conclusion!.agreedBy, ["B"])
  cli("master", ["task", "ack", "compare", "rev=4"])
  assert.deepEqual(snapshot().state.tasks[0]!.conclusion!.agreedBy, ["B"])
  cli("A", ["task", "agree", "compare", "rev=4"])
  ...
  assert.equal(state.tasks[0]!.attention, state.tasks[0]!.acknowledged)
...
test("non-pane child handles remain inspectable without weakening identity or ownership", () => {
  ...
  assert.equal((snapshot().state.tasks[0]!.conclusion?.disposition ?? "open"), "open")
})
...
test("user wait leaves independent work available and only master can resolve", () => {
  ...
  cli("A", ["dispatch", "reserve", "child-independent", "task=independent", "taskRev=1", "inspectAfter=2099-01-01T00:00:00Z"])
...
test("scope narrowing retains implementation as blocked work and restoration needs no extra authorization", () => {
  ...
  assert.match(cli("A", ["dispatch", "reserve", "child-fix", "task=fix", "taskRev=1", "inspectAfter=2099-01-01T00:00:00Z"], 1), /scope/)
  assert.match(cli("master", ["status"]), /fix/)
  cli("master", ["scope", "set", "rev=2", "mode=fix", "source=user resumed implementation"])
  cli("A", ["dispatch", "reserve", "child-fix", "task=fix", "taskRev=1", "inspectAfter=2099-01-01T00:00:00Z"])
})
...
test("dependencies retain both producers across checkout waits and reopenings", () => {
  const { cli, snapshot, directory } = fixture()
  cli("A", add("candidate")); cli("A", add("measurement"))
  cli("B", add("review", "dependsOn=candidate,measurement", "wait=checkout", "waitReason=use stable inputs"))
  cli("A", ["checkout", "take", "rev=0", "purpose=build"])
  const before = snapshot()
  for (const dependency of ["missing", "review", "candidate,candidate"]) {
    assert.match(cli("B", ["task", "set", "review", "rev=1", "dependsOn=" + dependency, "reason=fixture"], 1), /missing dependency|cycle|unique/)
  }
  assert.match(cli("A", ["task", "set", "candidate", "rev=1", "dependsOn=review", "reason=cycle"], 1), /cycle/)
  assert.deepEqual(snapshot(), before)
  const evidence = join(directory, "result.md")
  writeFileSync(evidence, "Provider unavailable; inspect this outcome before proceeding.")
  cli("A", ["task", "publish", "candidate", "rev=1", "disposition=stopped", "file=" + evidence])
  cli("A", ["checkout", "release", "rev=1", "reason=build stopped"])
  assert.doesNotMatch(cli("B", ["status"]), /task:review/)
  assert.match(cli("B", ["status", "review"]), /needs outcome from measurement/)
  cli("A", ["task", "publish", "measurement", "rev=1", "disposition=done", "file=" + evidence])
  assert.match(cli("B", ["status"]), /task:review/)
  assert.equal(snapshot().state.checkout, null, "availability does not acquire a resource")
  cli("A", ["task", "reopen", "candidate", "rev=2", "reason=provider restored"])
  assert.doesNotMatch(cli("B", ["status"]), /task:review/)
  assert.deepEqual(snapshot().state.tasks.find((task) => task.id === "review")!.dependsOn, ["candidate", "measurement"])
  assert.match(cli("B", ["task", "publish", "review", "rev=1", "disposition=done", "file=" + evidence], 1), /pending dependency/)
})

 test("removed task start is not required to publish and cannot create another state", () => {
  const { cli, snapshot } = fixture()
  cli("A", add("work"))
  const before = snapshot()
  assert.match(cli("A", ["task", "start", "work", "rev=1"], 1), /unknown command/)
  assert.deepEqual(snapshot(), before)
  assert.equal(Object.hasOwn(snapshot().state.tasks[0]!, "started"), false)
  assert.equal(Object.hasOwn(snapshot().state.tasks[0]!, "state"), false)
  cli("A", ["task", "publish", "work", "rev=1", "disposition=done", "file=" + source])
})
