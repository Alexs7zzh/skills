...
test("master read acknowledgement neither endorses nor erases later attention", () => {
  ...
  state = apply(state, { type: "task.agree", ...env("B"), id: "issue", rev: 3 })
  assert.ok(workSignals(state, "master").some((s) => s.key === "outcome:issue"))
  rejected(state, { type: "task.ack", ...env("master"), id: "issue", rev: 3 }, /rev/)
  state = apply(state, { type: "task.reopen", ...env("B"), id: "issue", rev: 4, reason: "new evidence refutes conclusion" })
  assert.equal(state.tasks[0]!.conclusion, null)
  assert.ok(workSignals(state, "master").some((s) => s.key === "outcome:issue"))
})
