...
Keep skill maintenance as the run's goal when using a project task as a trial. Before dispatch, retain a short trial brief with the **skill behavior under test**, **workload and permitted side effects**, and **stop condition**. Carry that brief into the master's and workers' contexts; a coordinator wake continues it, not a new standalone project assignment.
