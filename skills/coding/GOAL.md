...
3. **The project stays understandable.** A later reader has less context than the agent that wrote the code. Ownership, state changes, contracts, and failure outcomes should be clear from the code and its named interfaces. Complexity earns its place by serving a present obligation or reducing the reasoning needed to maintain the project.
4. **No wasted work or attention.** Within those values, remove waiting, repeated discovery, and ceremony. Code generation and reversal are cheap; regression risk, integration work, and human comprehension still cost something. A step earns its place by reducing a concrete uncertainty or preserving information, not because human-written code used to pass through it.

## How we think about the workflow

- **Information continuity.** The investigator develops the candidate while the code, experiment, and alternatives are still in context. A second reader judges the claim, evidence, and candidate together. Separate records preserve what changed; they do not impose approval of each intermediate thought. A session ending does not end the task or discard its evidence.
